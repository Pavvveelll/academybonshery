...
if ($user_email == "") {
  echo "Error. Parameters are missing <br />";
  return false;
}

// на всякий случай обнулим $FirstName и $LastName, если ничего не передано в скрипт
if ($FirstName == "") {
  $FirstName="";
}
...
$debug_1 = true;                                      // установить true если нужна отладочная информация на экран
...
// Проверяем есть ли пользователь в списке 1
$list_1_subscribed = "N";
if (is_subscribed($user_email, $debug_1, $api_key, $list_id_1, $data_center))
{
    // пользователь есть в $list_id_1, удаляем
    mc_unsubscribe($user_email, $api_key, $list_id_1, $data_center, $debug_1);
    //mc_delete ($user_email, $api_key, $list_id_1, $data_center, $debug_1);
    // и сообщаем входящему вебхуку, что пользователь был в $list_id_1
    $list_1_subscribed = "Y";
}
...
echo PHP_EOL.'list_1_subscribed: '.$list_1_subscribed;
